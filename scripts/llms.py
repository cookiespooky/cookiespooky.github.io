...
def main():
    ...
    # Страницы верхнего уровня собираются, а не перечисляются руками: иначе
    # каждая новая страница молча выпадает из карты, как выпала
    # «Как устроен этот сайт». collect отсеивает draft и noindex, то есть
    # страницы-редиректы сюда не попадают.
    for fm in sorted(collect("*.md", "page"), key=lambda f: f.get("nav_order", "99")):
        out.append(line(fm, f"{BASE}/{fm['slug']}/"))
