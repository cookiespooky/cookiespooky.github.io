---
type: article
id: "choice-paralysis"
title: "Паралич выбора"
slug: "choice-paralysis"
kind: "problem"
hub: "topics"
summary: "Ситуация, когда количество вариантов мешает перейти к действию."
description: "Ситуация, когда количество вариантов мешает перейти к действию."
status: "public"
lang: "ru"
links:
  - id: "cognitive-superposition"
    rel: "explained_by"
  - id: "loss-of-options"
    rel: "explained_by"
  - id: "no-selection-criterion"
    rel: "explained_by"
  - id: "product-choice"
    rel: "uses"
actions:
  - label: "Сформировать критерий"
    target: "cognitive-superposition"
    help: "Поможет уточнить контекст и перейти к следующему связанному понятию."
  - label: "Понять причину"
    target: "cognitive-superposition"
    help: "Поможет уточнить контекст и перейти к следующему связанному понятию."
  - label: "Перейти к продуктовой логике"
    target: "lead-product-choice"
    help: "Поможет выбрать один фокус и проверить его на спросе."
seo:
  title: "Паралич выбора"
  description: "Ситуация, когда количество вариантов мешает перейти к действию."
---
# Паралич выбора

Ситуация, когда количество вариантов мешает перейти к действию.

Связанные элементы графа: [[cognitive-superposition|Когнитивная суперпозиция]], [[loss-of-options|Страх потери вариантов]], [[no-selection-criterion|Нет критерия выбора]], [[product-choice|Выбор продукта]].

## Связи

- [[cognitive-superposition]] — explained_by
- [[loss-of-options]] — explained_by
- [[no-selection-criterion]] — explained_by
- [[product-choice]] — uses

## Следующие действия

- **Сформировать критерий** → [[cognitive-superposition]]: Поможет уточнить контекст и перейти к следующему связанному понятию.
- **Понять причину** → [[cognitive-superposition]]: Поможет уточнить контекст и перейти к следующему связанному понятию.
- **Перейти к продуктовой логике** → [[lead-product-choice]]: Поможет выбрать один фокус и проверить его на спросе.
