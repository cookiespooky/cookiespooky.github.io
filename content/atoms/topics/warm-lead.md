---
type: article
id: "warm-lead"
title: "Теплый лид"
slug: "warm-lead"
kind: "persona"
hub:
  - "topics"
summary: "Человек, который уже проявил интерес к теме, проблеме, материалу или действию. С ним важно продолжать диалог из его контекста, а не начинать продажу с нуля."
description: "Человек, который уже проявил интерес к теме, проблеме, материалу или действию. С ним важно продолжать диалог из его контекста, а не начинать продажу с нуля."
status: "public"
lang: "ru"
links:
  - id: "comment-to-dm-flow"
    rel: "produces"
  - id: "sales-conversation"
    rel: "leads_to"
  - id: "offer"
    rel: "related_to"
  - id: "objections-map"
    rel: "related_to"
actions:
  - label: "Подготовить диалог"
    target: "sales-conversation"
    help: "Поможет перейти от интереса к пониманию ситуации."
  - label: "Проверить оффер"
    target: "offer"
    help: "Поможет предложить следующий шаг без давления."
  - label: "Собрать карту заявок"
    target: "lead-content-map"
    help: "Поможет определить, какие сигналы считать лидовыми."
seo:
  title: "Теплый лид"
  description: "Человек, который уже проявил интерес к теме, проблеме, материалу или действию. С ним важно продолжать диалог из его контекста, а не начинать продажу с нуля."
---
# Теплый лид

Человек, который уже проявил интерес к теме, проблеме, материалу или действию. С ним важно продолжать диалог из его контекста, а не начинать продажу с нуля.

Связанные элементы графа: [[comment-to-dm-flow|Переход из комментария в личку]], [[sales-conversation|Продающий диалог]], [[offer|Оффер]], [[objections-map|Карта возражений]].

## Связи

- [[comment-to-dm-flow|Переход из комментария в личку]] — produces
- [[sales-conversation|Продающий диалог]] — leads_to
- [[offer|Оффер]] — related_to
- [[objections-map|Карта возражений]] — related_to

## Следующие действия

- **Подготовить диалог** → [[sales-conversation|Продающий диалог]]: Поможет перейти от интереса к пониманию ситуации.
- **Проверить оффер** → [[offer|Оффер]]: Поможет предложить следующий шаг без давления.
- **Собрать карту заявок** → [[lead-content-map|Собрать карту контента и заявок]]: Поможет определить, какие сигналы считать лидовыми.
