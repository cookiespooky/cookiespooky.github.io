---
type: article
id: "case-tarot-client-acquisition"
title: "Кейс таролога с повторными клиентами"
slug: "case-tarot-client-acquisition"
kind: "case"
hub:
  - "research"
summary: "Кейс специалиста, у которого есть повторные оплаты и доверие, но нет устойчивой системы привлечения новых клиентов."
description: "Кейс специалиста, у которого есть повторные оплаты и доверие, но нет устойчивой системы привлечения новых клиентов."
status: "public"
lang: "ru"
links:
  - id: "offer"
    rel: "uses"
  - id: "threads"
    rel: "related_to"
  - id: "warm-lead"
    rel: "produces"
  - id: "content-to-leads"
    rel: "explains"
  - id: "lead-content-map"
    rel: "leads_to"
actions:
  - label: "Собрать карту контента"
    target: "lead-content-map"
    help: "Поможет превратить темы постов в маршрут к заявке."
  - label: "Посмотреть переход в личку"
    target: "comment-to-dm-flow"
    help: "Поможет продолжать интерес без резкой продажи."
---
# Кейс таролога с повторными клиентами

Кейс показывает разрыв между качеством услуги и системой привлечения. Повторные обращения подтверждают ценность продукта, но входящий поток зависит от знакомых, рекомендаций и нерегулярного контента. Для роста нужны понятный [[offer|Оффер]], маршрут из [[threads|Threads]] в диалог и карта тем, которые связывают запрос клиента с услугой.

Связанные элементы графа: [[offer|Оффер]], [[threads|Threads]], [[warm-lead|Теплый лид]], [[content-to-leads|Контент в заявки]], [[lead-content-map|Собрать карту контента и заявок]].

## Связи

- [[offer|Оффер]] — uses
- [[threads|Threads]] — related_to
- [[warm-lead|Теплый лид]] — produces
- [[content-to-leads|Контент в заявки]] — explains
- [[lead-content-map|Собрать карту контента и заявок]] — leads_to

## Следующие действия

- **Собрать карту контента** → [[lead-content-map|Собрать карту контента и заявок]]: Поможет превратить темы постов в маршрут к заявке.
- **Посмотреть переход в личку** → [[comment-to-dm-flow|Переход из комментария в личку]]: Поможет продолжать интерес без резкой продажи.
