---
id: diary
title: Дневник
slug: diary
kind: practice
hub: tools
summary: Поток личных наблюдений, событий, мыслей и решений, который со временем становится
  материалом для анализа.
status: public
links:
- id: obsidian
  rel: compatible_with
- id: self-tracking
  rel: uses
- id: repeating-scenarios
  rel: related_to
- id: cognitive-reconstruction
  rel: uses
- id: lead-notes-analysis
  rel: leads_to
- id: daily-notes
  rel: contains
- id: self-tracking-signals
  rel: contains
- id: personal-knowledge-base
  rel: part_of
- id: state-log
  rel: contains
- id: decision-log
  rel: contains
- id: weekly-review
  rel: uses
- id: trigger-map
  rel: produces
actions:
- label: Проанализировать дневник
  target: obsidian
  help: Поможет уточнить контекст и перейти к следующему связанному понятию.
- label: Связать с графом
  target: obsidian
  help: Поможет уточнить контекст и перейти к следующему связанному понятию.
- label: Найти повторения
  target: obsidian
  help: Поможет уточнить контекст и перейти к следующему связанному понятию.
- label: Разобрать мои заметки
  target: lead-notes-analysis
  help: Поможет превратить хаотичные заметки в атомы, связи и карту тем.
- label: Собрать self-tracking из дневника
  target: lead-self-tracking-setup
  help: Поможет превратить дневниковый поток в простую систему наблюдений.
seo:
  title: Дневник
  description: Поток личных наблюдений, событий, мыслей и решений, который со временем становится
    материалом для анализа.
---

# Дневник

Поток личных наблюдений, событий, мыслей и решений, который со временем становится материалом для анализа.

Связанные элементы графа: [[obsidian]], [[self-tracking]], [[repeating-scenarios]], [[cognitive-reconstruction]], [[lead-notes-analysis]], [[daily-notes]], [[self-tracking-signals]], [[personal-knowledge-base]], [[state-log]], [[decision-log]], [[weekly-review]], [[trigger-map]].

## Связи

- [[obsidian]] — compatible_with
- [[self-tracking]] — uses
- [[repeating-scenarios]] — related_to
- [[cognitive-reconstruction]] — uses
- [[lead-notes-analysis]] — leads_to
- [[daily-notes]] — contains
- [[self-tracking-signals]] — contains
- [[personal-knowledge-base]] — part_of
- [[state-log]] — contains
- [[decision-log]] — contains
- [[weekly-review]] — uses
- [[trigger-map]] — produces

## Следующие действия

- **Проанализировать дневник** → [[obsidian]]: Поможет уточнить контекст и перейти к следующему связанному понятию.
- **Связать с графом** → [[obsidian]]: Поможет уточнить контекст и перейти к следующему связанному понятию.
- **Найти повторения** → [[obsidian]]: Поможет уточнить контекст и перейти к следующему связанному понятию.
- **Разобрать мои заметки** → [[lead-notes-analysis]]: Поможет превратить хаотичные заметки в атомы, связи и карту тем.
- **Собрать self-tracking из дневника** → [[lead-self-tracking-setup]]: Поможет превратить дневниковый поток в простую систему наблюдений.
