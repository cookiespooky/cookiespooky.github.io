---
id: analysis-lens
title: Аналитическая линза
slug: analysis-lens
kind: concept
hub: methods
summary: Рамка анализа, которая задает, какие элементы ситуации считать значимыми
  и какие вопросы к ним применять.
status: public
links:
- id: lens-analysis
  rel: part_of
- id: lat
  rel: example_of
- id: marketing-os
  rel: example_of
- id: product-thinking
  rel: example_of
- id: cognitive-reconstruction
  rel: uses
actions:
- label: Понять линзирование
  target: lens-analysis
  help: Поможет увидеть, как разные рамки меняют прочтение одной ситуации.
- label: Посмотреть LAT как линзу
  target: lat
  help: Поможет увидеть языковую рамку анализа.
- label: Посмотреть продуктовую линзу
  target: product-thinking
  help: Поможет анализировать ситуацию через продуктовые решения.
seo:
  title: Аналитическая линза
  description: Рамка анализа, которая задает, какие элементы ситуации считать значимыми
    и какие вопросы к ним применять.
---

# Аналитическая линза

Аналитическая линза задает фокус. Она не заменяет ситуацию, а выбирает способ ее чтения.

В одной ситуации можно применять несколько линз: языковую, продуктовую, маркетинговую, сценарную, поведенческую или философскую.

Связанные элементы графа: [[lens-analysis]], [[lat]], [[marketing-os]], [[product-thinking]], [[cognitive-reconstruction]].

## Связи

- [[lens-analysis]] — part_of
- [[lat]] — example_of
- [[marketing-os]] — example_of
- [[product-thinking]] — example_of
- [[cognitive-reconstruction]] — uses

## Следующие действия

- **Понять линзирование** → [[lens-analysis]]: Поможет увидеть, как разные рамки меняют прочтение одной ситуации.
- **Посмотреть LAT как линзу** → [[lat]]: Поможет увидеть языковую рамку анализа.
- **Посмотреть продуктовую линзу** → [[product-thinking]]: Поможет анализировать ситуацию через продуктовые решения.
