...
kind: method
hub: methods
summary: AI-рефакторинг — изменение структуры кода с помощью модели, при котором сохраняется
  поведение системы, а человек проверяет цель, риски и результат.
status: public
links:
- id: codex
  rel: implemented_by
...
- label: Проверить проект на рефакторинг
  ...
  help: Поможет понять, где модель может безопасно упростить код, а где лучше не трогать
    проводку под напряжением.
---
