...
kind: method
hub: methods
summary: AI-dev workflow — порядок работы, в котором человек формулирует цель, готовит
  контекст, модель меняет код, а результат проверяется короткими итерациями.
status: public
links:
- id: vibe-coding
  rel: explains
